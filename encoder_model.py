...
def torch_padding(downsample, kernel_size):
    """Compute required padding"""
    padding = max(0, int(np.floor((kernel_size - downsample + 1) / 2)))
    return padding


def torch_downsample(n_samples_in, n_samples_out):
...
class PTResBlock1d(nn.Module):
    """Residual network unit for unidimensional signals."""

    def __init__(self, n_filters_in, n_filters_out, downsample, kernel_size, dropout_rate):
        if kernel_size % 2 == 0:
            raise ValueError("The current implementation only support odd values for `kernel_size`.")
        super(PTResBlock1d, self).__init__()
        # Forward path
        padding = torch_padding(1, kernel_size)
        self.conv1 = nn.Conv1d(n_filters_in, n_filters_out, kernel_size, padding=padding, bias=False)
        self.bn1 = nn.BatchNorm1d(n_filters_out)
        self.relu = nn.ReLU()
        self.dropout1 = nn.Dropout(dropout_rate)
        padding = torch_padding(downsample, kernel_size)
        ...
        if n_filters_in != n_filters_out:
            conv1x1 = nn.Conv1d(n_filters_in, n_filters_out, 1, bias=False) # (1,1)
...
class PTResNet1d(nn.Module):
    ...
    -----------
    References: https://github.com/antonior92/automatic-ecg-diagnosis and https://github.com/antonior92/ecg-age-prediction
    """

    def __init__(self, input_dim, blocks_dim, n_classes, kernel_size=17, dropout_rate=0.8):
        super(PTResNet1d, self).__init__()
        # First layers
        n_filters_in, n_filters_out = input_dim[0], blocks_dim[0][0]
        n_samples_in, n_samples_out = input_dim[1], blocks_dim[0][1]
        downsample = torch_downsample(n_samples_in, n_samples_out)
        padding = torch_padding(downsample, kernel_size)
        ...
        for i, (n_filters, n_samples) in enumerate(blocks_dim):
            ...
            downsample = torch_downsample(n_samples_in, n_samples_out)
            resblk1d = PTResBlock1d(n_filters_in, n_filters_out, downsample, kernel_size, dropout_rate)
